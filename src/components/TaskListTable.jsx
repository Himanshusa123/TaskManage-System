...
const TaskListTable = ({tabledata}) => {
  const getstatusbadegcolor = (status) => {
    switch (status) {
      case "completed":
        ...
        <tbody>
          ...
          {tabledata.map((task)=>(
            <tr key={task._id} className="border-t border-gray-200">
              <td className="my-3 mx-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden">{task.title
              }</td>
              <td className="py-4 px-4"
              ><span  className={`px-2 py-1 text-xs rounded inline-block ${getstatusbadegcolor(
                task.status
              )}`}>{task.status}</span>
