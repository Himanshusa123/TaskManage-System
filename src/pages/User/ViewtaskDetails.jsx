...
const ViewtaskDetails = () => {
  ...
  const updatetodochecklist=async(index)=>{
    ...
    if (todochecklist &&todochecklist[index]) {
      ...
      try {
        ...
        const res=await axiosInstance.put(API_PATHS.TASK.UPDATE_TODO_CHECKLIST(taskId),{
          todochecklist
        });
        if (res.status===200) {
  ...
  const handleattachmentlink=(link)=>{
        ...
        {task?.attachments?.length > 0 &&(
          <div className='mt-2'>
            ...
            {task?.attachments?.map((link,index)=>(
              <Attachment key={`link_${index}`} link={link} index={index} onClick={()=>handleattachmentlink} />
            ))}
          </div>
        )}
      </div>
